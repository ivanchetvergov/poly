...
bool RBTree::insert(const std::string& word) {
    if (!m_root){
    ...
    if (word < parent->key) parent->left = std::move(newNode);
...
bool RBTree::search(const std::string& word) const {
    RBNode* curr = m_root.get();
    while (curr) {
        if (word == curr->key) return true;
        if (word < curr->key) curr = curr->left.get();
        else curr = curr->right.get();
    }
    return false;
}

RBNode* RBTree::findNode(const std::string& word) const {
    RBNode* curr = m_root.get();
    while (curr) {
        if (word == curr->key) return curr;
        if (word < curr->key) curr = curr->left.get();
        else curr = curr->right.get();
    }
    return nullptr;
...
bool RBTree::remove(const std::string& word) {
    ...
    if (!node->left && !node->right) {
        fixupNode = nullptr;
        std::unique_ptr<RBNode>& nodeRef = (node->parent ?
...
std::pair<RBNode*, RBNode*> RBTree::findInsertPosition(const std::string& word) const {
    RBNode* curr = m_root.get();
    RBNode* parent = nullptr;
    while (curr) {
        parent = curr;
        if (word == curr->key) return {parent, curr};
        if (word < curr->key) curr = curr->left.get();
        else curr = curr->right.get();
    }
...
void RBTree::preOrderVisit(const std::unique_ptr<RBNode>& node) const {
    if (!node) return;
...
void RBTree::inOrderVisit(const std::unique_ptr<RBNode>& node) const {
    if (!node) return;
...
void RBTree::postOrderVisit(const std::unique_ptr<RBNode>& node) const {
    if (!node) return;
...
void RBTree::rotateLeft(RBNode* pt) {
    ...
    pt->parent = (pt->parent ? (pt == pt->parent->left.get() ? pt->parent->left.get() : pt->parent->right.get()) : m_root.get());
}

void RBTree::rotateRight(RBNode* pt) {
    if (!pt || !pt->left) return;
...
bool RBTree::loadFromFile(const std::string& filename) {
...
bool RBTree::saveToFile(const std::string& filename) const {
    std::ofstream file(filename);
    if (!file.is_open()) return false;
    std::function<void(const std::unique_ptr<RBNode>&)> saveInOrder = [&](const std::unique_ptr<RBNode>& node) {
        if (!node) return;
