...
показана финальная визуализация потока из вершины~$1$ в вершину~$7$.
...
\subsubsection{Исходный граф}

В качестве исходного используется связный неориентированный взвешенный
граф из лабораторной работы~1. Граф и матрица его смежности показаны на
...
Для графа из~11 вершин последовательность имеет длину~9:
\[
    3\ 5\ 3\ 1\ 2\ 10\ 4\ 4\ 0.
\]
...
% Программа проверяет наличие вершин нечётной степени и связность графа.
% При несоблюдении условий теоремы Эйлера выполняется достройка~---
% каждое добавленное ребро логируется в консоль. Затем алгоритмом
% Иерхольцера строится эйлеров цикл или путь. Результат показан на
% рис.~\ref{fig:euler_graph} и~\ref{fig:euler_output}.

% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=0.82\linewidth]{images/lab5/euler_graph.png}
...
% Программа строит МОД алгоритмом Борувки и для каждого ребра остова
% вычисляет фундаментальный разрез: BFS-обходом без удалённого ребра
% находится компонента~$A$, определяется дополнение~$B$ и выводятся
% все рёбра исходного графа, пересекающие границу $A \mid B$.
% Визуализация МОД приведена на рис.~\ref{fig:lab5_mst}; консольный
% вывод ФСР~--- на рис.~\ref{fig:cuts_output}.

% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=0.82\linewidth]{images/lab5/mst.png}
...
% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=0.7\linewidth]{images/lab5/cuts_output.png}
%     \caption{Консольный вывод: фундаментальная система разрезов}
%     \label{fig:cuts_output}
% \end{figure}

% \subsubsection{Симметрическая разность подмножества разрезов}

% Пользователь вводит индексы разрезов из ФСР. Программа вычисляет
% симметрическую разность: для каждого ребра выводится число его
% вхождений в выбранные разрезы и итоговое решение~--- включить
% или исключить. Результирующее множество рёбер выводится в консоль
% и выделяется на графе (рис.~\ref{fig:symdiff_graph}
% и~\ref{fig:symdiff_output}).

% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=0.82\linewidth]{images/lab5/symdiff_graph.png}
...
% \begin{figure}[H]
%     \centering
%     % \includegraphics[width=0.6\linewidth]{images/lab5/symdiff_output.png}
