...
- Через Jupyter: File > Export > PDF (нужен Pandoc + LaTeX).
- Или экспорт в HTML (`jupyter nbconvert --to html examples.ipynb`), потом браузер > Печать > Сохранить как PDF.
...
- Генерация сетей потоков.
- Алгоритм Форда-Фалкерсона для макс потока.
- Минимальная стоимость потока.
- Визуализация: сеть с capacity/flow, анимация роста потока (GIF).

### Lab 4: Остовы и комбинаторика

- Число остовных деревьев (матрица Кирхгофа).
- Независимые множества вершин/рёбер.
- Минимальные вершинные/рёберные покрытия.
- Минимальная раскраска графа.
- Визуализация: раскрашенные графы, покрытия.

### Lab 5: Циклы

- Проверка на эйлеровы/гамильтоновы циклы.
- TSP (приближённое решение).
- Визуализация: графы с циклами.

### Lab 6: Структуры данных

- Хеш-таблица, красно-чёрное дерево.
...
Пул-реквесты welcome! Пиши на C++20, добавляй тесты, обновляй ноутбук. Если добавишь новую лабу — обнови examples.ipynb.

Блин, это реально круто для учёбы — интеграция на уровне, преподу покажи ноутбук, он ахнет. 🚀
